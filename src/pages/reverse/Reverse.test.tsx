...
describe('Reverse Component', () => {
  ...
  describe('Context Resolution - breadcrumbsProp path', () => {
    ...
    it('should use reverseTransactionApi when breadcrumbsProp.path is BUY_MANAGEMENT', () => {
      ...
      expect(screen.getByTestId('breadcrumb-label')).toHaveTextContent('Buy Management');
    ...
    it('should fallback to reverseTransactionApi when breadcrumbsProp.path is unknown', () => {
      ...
      expect(screen.getByTestId('breadcrumb-path')).toHaveTextContent('/unknown-route');
      expect(screen.getByTestId('api-call')).toHaveTextContent('reverseTransactionApi');
    });
  });
  ...
  describe('FileUploadAction Props', () => {
    ...
    it('should pass correct static props to FileUploadAction', () => {
      ...
      expect(screen.getByTestId('title-key')).toHaveTextContent('pages.reverse.title');
      expect(screen.getByTestId('subtitle-key')).toHaveTextContent('pages.reverse.subtitle');
      expect(screen.getByTestId('i18n-block-key')).toHaveTextContent('pages.reverse');
      expect(screen.getByTestId('success-state-key')).toHaveTextContent('reverseUploadSuccess');
    });
    ...
    it('should pass environment variable manualLink', () => {
      ...
      expect(screen.getByTestId('manual-link')).toHaveTextContent('https://manual.example.com');
    });

    it('should pass correct styleClass from CSS module', () => {
      render(<Reverse />);

      expect(screen.getByTestId('style-class')).toHaveTextContent('uploadFileContainer');
    });
