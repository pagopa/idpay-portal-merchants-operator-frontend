...
describe('Merchant Service Functions', () => {
  ...
  describe('authPaymentBarCode', () => {
    ...
    it('should propagate errors from MerchantApi.authPaymentBarCode', async () => {
      ...
      await expect(authPaymentBarCode(mockParams)).rejects.toThrow('Transaction not found');
      expect(MerchantApi.authPaymentBarCode).toHaveBeenCalledWith(mockParams);
    });
  });
  ...
  describe('getProcessedTransactions', () => {
    ...
    it('should propagate errors from MerchantApi.getProcessedTransactions', async () => {
      ...
      vi.mocked(MerchantApi.getProcessedTransactions).mockRejectedValue(mockError);
  ...
  describe('Integration scenarios', () => {
    it('should handle concurrent calls to different service functions', async () => {
      ...
      vi.mocked(MerchantApi.getProducts).mockResolvedValue(mockProductsResponse);
      vi.mocked(MerchantApi.previewPayment).mockResolvedValue(mockPreviewResponse as any);
    ...
    it('should maintain function isolation on errors', async () => {
      vi.mocked(MerchantApi.getProducts).mockRejectedValue(new Error('Products API Error'));
      vi.mocked(MerchantApi.previewPayment).mockResolvedValue({ trxId: 'TRX123', status: 'OK' } as any);

      await expect(getProductsList({ page: 1 })).rejects.toThrow('Products API Error');
