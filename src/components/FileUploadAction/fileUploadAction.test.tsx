...
describe('fileUploadAction component test', () => {
  ...
  it('should display file type error alert', () => {
    const { getByTestId, queryByTestId } = render(<FileUploadAction {...baseProps} />);
  ...
  it('should clear the file when the remove button is clicked', async () => {
    ...
    const fileInput = screen.getByTestId('upload-input-test') as HTMLInputElement;
    ...
    expect(screen.getByText(`${baseProps.i18nBlockKey}.uploadFile`)).toBeInTheDocument();
  });
