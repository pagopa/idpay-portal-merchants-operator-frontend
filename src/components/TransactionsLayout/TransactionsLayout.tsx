...
const TransactionsLayout: React.FC<TransactionsLayoutProps> = ({
  ...
  const [paginationModel, setPaginationModel] =
    useState<PaginationExtendedModel>({
      page: 0,
  ...
  const handleApplyFilters = (filtersObj: GetProcessedTransactionsFilters) => {
    ...
    if (
      sortModel?.length > 0 &&
      sortModel[0].field === "additionalProperties"
    ) {
      fetchTransactions({
    ...
    <Box>
            ...
            <Grid size={{ xs: 12, sm: 6, md: 3, lg: 3 }}>
              <FormControl fullWidth size="small">
                <InputLabel id="status-label">
                  {t("commons.statusFilterPlaceholer")}
                </InputLabel>
                <Select
      ...
      {errorAlert && (
        <AlertComponent
          error={true}
          message={
            alertMessages.error || t("pages.refundManagement.errorAlert")
          }
        />
      )}
