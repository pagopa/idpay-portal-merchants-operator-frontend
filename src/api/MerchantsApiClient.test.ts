...
describe('MerchantApi', () => {
  ...
  describe('invoiceTransactionApi', () => {
    ...
    it('should throw an errore when API call fail', async () => {
      ...
      await expect(MerchantApi.invoiceTransactionApi(trxId, testFile, 'DOC789')).rejects.toThrow(
  ...
  describe('deleteTransactionInProgress', () => {
    it('should call DELETE /transactions/:trxId with correct transaction ID', async () => {
      ...
      await expect(MerchantApi.deleteTransactionInProgress(trxId)).resolves.toEqual({});

      expect(mockAxiosInstance.delete).toHaveBeenCalledWith(`/transactions/${trxId}`);
